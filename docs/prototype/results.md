...
- **#30 — Claude Code headless fallback for V1.** Resolved:
  `_v1_client` now dispatches to the `claude` CLI when
  `ANTHROPIC_API_KEY` is unset, so V1 runs in subscription-only
  environments. Cost-axis caveat: CLI mode returns `total_cost_usd`
  per call but timings are not yet stamped into `EvalReport`; treat
  CLI-vs-SDK latency comparisons as advisory until that backend
  marker lands as a follow-up.
