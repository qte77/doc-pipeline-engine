...
Rows carry a trailing `*(added YYYY-MM-DD)*` or `*(verified YYYY-MM-DD)*` marker recording when that row's facts were last checked against first-party sources. The frontmatter date covers the file; these cover the individual row. A row with no marker predates the convention and should be treated as unverified since the frontmatter `updated` date.
...
| **Unstructured.io** ([repo](https://github.com/Unstructured-IO/unstructured)) | Apache-2.0 (OSS core) + commercial API | Ingest + element-typed extraction + chunking; downstream RAG via partner libs | **Adjacent** — now also a candidate extraction backend in [ingest.md §1](ingest.md#1-extraction-backends) and a candidate processor (see [process.md](process.md)). Scope is ingest+extract; we cover all five stages with strict contracts. *(verified 2026-08-09)* |
| **RAGFlow** ([repo](https://github.com/infiniflow/ragflow)) | Apache-2.0 | Deep-document-understanding RAG engine with agent capabilities; ingest → Elasticsearch/Infinity-backed hybrid search → agentic QA; full web platform | **Active competitor at scale** — 87k stars, v0.26.4 (2026-07-07), still shipping weekly (last push 2026-08-08). Same deep-document-understanding + agentic-RAG E2E scope as R2R and Knowhere, but ~10× R2R's star count and not stalled. Tempers the "competitor field has stalled" reading of §5.1: the embeddable-vs-service differentiator still holds (RAGFlow is a deployed platform), but the velocity argument does not. *(added 2026-08-09)* |
...
| **Kreuzberg** ([repo](https://github.com/kreuzberg-dev/kreuzberg)) | Elastic License 2.0 (ELv2; GitHub API returns NOASSERTION — confirmed by reading LICENSE) | Async multi-format extraction facade; covers PDF, Office, images, email, HTML in one API | **Adjacent extraction layer** — v4.9.8 (2026-05-25), 8.9k stars. Already the primary breadth adapter in [ingest.md §1](ingest.md#1-extraction-backends). ELv2 restricts SaaS redistribution; same Tier-G treatment as documented in ingest.md and [issue #76](https://github.com/qte77/doc-pipeline-engine/issues/76). **Upstream moved:** the repo now redirects to `xberg-io/xberg` (MIT, Rust core, v1.x), with an MIT `kreuzberg-dev/kreuzberg-lts` for the v4 line — see the ingest.md open question. *(verified 2026-08-09)* |
...
1. **Embeddable engine, not a service.** R2R, AWS IDP, Azure DI, Document AI are services. We ship as a Python library with a CLI; orchestrators (polyforge, office-polyforge, Claude Code plugins) embed us — there is no server to run. *Partly strengthened* by the R2R maintenance stall (last commit 2025-11-07) — the closest service-shaped competitor has stalled. But the claim rests on *embeddability*, not on competitor decay: RAGFlow (Apache-2.0, 87k stars, shipping weekly) is a faster-moving competitor at similar E2E scope (§2), so do not lean on "the field has stalled" as supporting evidence.
...
3. **Strict license isolation.** Apache-2.0 default; AGPL (PyMuPDF), Tier-G OpenRAIL-M **weights** (marker/surya/Chandra — marker's own code relicensed GPL-3.0 → Apache-2.0 at v2.0.0, 2026-07-20, so only the bundled weights remain gated), ELv2 (Kreuzberg), and JVM (Tika, veraPDF) only as opt-in extras. R2R, Unstructured, and the AWS samples are mostly Apache-2.0/MIT too, but none of them publish a license-isolation policy as a first-class architectural commitment.
