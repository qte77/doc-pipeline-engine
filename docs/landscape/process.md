...
| **GLiNER** ([repo](https://github.com/urchade/GLiNER)) | Generative LM-based zero-shot NER | Apache-2.0 | Python + torch (~500 MB) | local | **Candidate (Comprehensive)** — v0.2.28 (2026-07-24), 3.5k stars. Domain-specific entities without retraining; gate behind `[ner-gliner]`. Siblings: **GLiREL** ([repo](https://github.com/jackboyla/GLiREL)) for zero-shot relation extraction (MIT, v1.2.1, 274 stars); **gliner-multitask** model (HuggingFace: `knowledgator/gliner-multitask-large-v0.5`) for NER + RE + classification in one pass — no separate repo, load via GLiNER library. Two independent successors to evaluate: **GLiNER2** ([repo](https://github.com/fastino-ai/GLiNER2)) — first-party (Fastino AI) 205M CPU-only model consolidating GLiNER + GLiREL + gliner-multitask into one model and one dependency (Apache-2.0, v1.3.2, ~1.7k stars, EMNLP 2025); also overlaps §6. And **GLiFormer** (see row below) — Knowledgator's own consolidation attempt. Worth a three-way consolidation benchmark at [§0.4.0](../roadmap.md#040--adapters) before either displaces the trio above — tracked in [issue #197](https://github.com/qte77/doc-pipeline-engine/issues/197). *(verified 2026-08-09; GLiNER2 added 2026-08-09)* |
| **GLiFormer** ([repo](https://github.com/Knowledgator/GLiFormer)) | Schema-conditioned encoder: NER + classification + joint relation extraction + nested-JSON structuring + embeddings in one DeBERTa-based model (base 264M / large 576M); labels/schema passed at inference time, no autoregressive decoding. Dedicated layout checkpoint extracts fields from a PDF using **both text and word bounding boxes** (`layout_model.parse_pdf()`, coords normalized 0–1000) — a direct PDF+bbox path, not just plain-text extraction | Apache-2.0 | Python + torch (`pip install gliformer`); CPU or GPU | local | **Candidate (Comprehensive)** — released 2026-09-16 by Knowledgator (same org as `gliner-multitask` above). Vendor-reported 91.10 F1 (large) on order-free/boundary-tolerant nested-JSON extraction, competitive with GPT-5-mini — no independent benchmark yet and no long-document/large-table stress test comparable to ExtractBench (§6). Confirmed gap (repo README, 2026-09-19): **no table or technical-drawing extraction** mentioned anywhere — test explicitly before relying on it for tabular data. Overlaps GLiNER2's consolidation goal (see GLiNER row); also see §6 below for its Pydantic-native schema-extraction head. Its PDF+bbox layout path may overlap docling's layout-ownership role ([ADR-0011](../adr/0011-content-layout-owned-by-docling.md)) rather than complement it — resolve ownership if this is ever wired. Benchmark tracked in [issue #197](https://github.com/qte77/doc-pipeline-engine/issues/197). *(verified against repo README 2026-09-19)* |
...
| **GLiFormer** (cross-ref §3) | Nested-JSON schema-conditioned structuring head — encoder, non-generative; schema/labels passed at inference, order-free/boundary-tolerant matching. Structured output validates **directly against nested Pydantic models** (e.g. company → departments → employees in one call) | Apache-2.0 | Python + torch; CPU or GPU | local | **Candidate** — full profile in §3. Its non-generative single-pass architecture sidesteps NuExtract3's ExtractBench truncation failure mode on long documents *by construction*, but that is architectural reasoning, not a tested claim — GLiFormer's own published benchmark is a 500-example nested-JSON set, not a long-document/enormous-table stress test. Native Pydantic validation is a lower integration cost than NuExtract3's `convert_json_schema_to_nuextract_template()` adapter or LangExtract's `ExampleData`/`Extraction` adapter — worth weighing alongside the accuracy comparison, not just F1. Benchmark alongside NuExtract3/LangExtract at [§0.4.0](../roadmap.md#040--adapters) before drawing conclusions — tracked in [issue #197](https://github.com/qte77/doc-pipeline-engine/issues/197). *(verified against repo README 2026-09-19)* |
| **Jev / TypeSafe System One** ([site](https://www.jevai.org/), [announcement](https://typesafe.ai/blog/introducing-system-one-models-and-jev)) | Typed bounded-label decisions with calibrated confidence, single parallel pass (not schema-conditioned JSON extraction from unbounded text) | Proprietary | Cloud API only; early access / waitlist | cloud-only | **Avoid** — proprietary, API-only, no self-hosting, no published pricing/SLA, no independent benchmark confirmation. Task shape is classification/routing over a known bounded label set (its own published cases: intent routing, invoice classification, content moderation, claim support/contradict/no-mention scoring), not open-ended extraction from document text — not a substitute for anything in this table. Speculative future fit only (e.g. scoring `AnalysisReport.claims` as supported/contradicted/no-mention, or Quick/Comprehensive tier routing) if a pipeline gap ever forces it. Don't-adopt decision tracked in [issue #196](https://github.com/qte77/doc-pipeline-engine/issues/196). *(added 2026-09-19)* |
...
- Should GLiNER2 or GLiFormer replace the GLiNER + GLiREL + gliner-multitask trio (one model, one dependency), or run alongside it? Two independent consolidation candidates now exist (GLiNER2: Fastino AI, 205M, CPU-only; GLiFormer: Knowledgator, 264M/576M, adds a nested-JSON structuring head overlapping §6). Recommended default: benchmark all three approaches at §0.4.0 before consolidating — tracked in [issue #197](https://github.com/qte77/doc-pipeline-engine/issues/197).
