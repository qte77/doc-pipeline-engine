...
- `.github/workflows/generate-sbom.yaml` — revert `qte77/gha-sbom-action` v0.1.1 → **v0.1.0**. v0.1.1 commits the SBOM PR as an **unsigned** `github-actions` commit, which the repo's `required_signatures` ruleset (no bypass actors) rejects → every SBOM auto-PR became unmergeable, even with `--admin` (closed #146/#148). v0.1.0 produces a signed (API) commit. Added a `github-actions` dependabot `ignore` for `>=0.1.1` so it isn't re-bumped; upstream fix is for the action to sign its commit.
